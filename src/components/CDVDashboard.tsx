...
import { Brain, Users, TrendingUp, CheckCircle, RefreshCw } from 'lucide-react'
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Attendee ID</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {reports.map((report) => (
                  <tr key={report.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{report.attendee_id}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{report.zone_name || 'HVZ-1'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{report.dwell_time_minutes}m</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                        report.active_engagement_status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
