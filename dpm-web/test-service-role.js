...
async function createTestDataWithServiceRole() {
  ...
  try {
    // Step 0: Create organizer user first
    console.log('0️⃣ Creating organizer user...');
    let organizer;
    const organizerId = randomUUID();
    const { data: newOrganizer, error: organizerError } = await supabaseAdmin
      .from('profiles')
      .insert({
        id: organizerId,
        email: 'organizer@naveaze.co.za',
    ...
    if (organizerError) {
      console.log('⚠️  Organizer might already exist, fetching existing...');
      const { data: existingOrganizer } = await supabaseAdmin
        .from('profiles')
        .select('*')
        .eq('email', 'organizer@naveaze.co.za')
        .single();
      ...
      console.log(`✅ Organizer created: ${organizer.first_name} ${organizer.last_name} (ID: ${organizer.id})`);
    }
