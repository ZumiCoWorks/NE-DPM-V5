...
export function ScannerScreen({ onScanSuccess }: ScannerScreenProps) {
  ...
  const handleTicketOrQR = async (raw: string) => {
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/functions/v1/get-quicket-lead`, {
        method: 'POST',
  ...
  const handleDemoScan = () => {
    onScanSuccess({ name: 'John Doe', email: 'john.doe@email.com' });
  };
